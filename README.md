...
- **`-LiveData`** - a PC off the work network (the dev laptop). The same files come through the private `flowstate-live-data` GitHub repo every 30 min; the first pull opens a GitHub sign-in. To run such a PC **exactly like a planner's PC**, use `-FeedDir "<path>\fs_data" -MirrorGitHub` instead (2026-09-18): the pull drops the GitHub clone's files into that folder's `fs_vif` / `fs_manual` (only when GitHub's copy is newer; nothing deleted) and then syncs from the folder as a planner's PC does.
