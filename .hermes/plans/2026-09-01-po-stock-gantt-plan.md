...
**Status:** BUILT 2026-09-01 (slices 0, 1, 2a, 2b) with every decision at its recommended default; **slice 3 BUILT 2026-09-15** (user: "build slice 3") — contract §10 in `2026-09-01-po-stock-contracts.md`: `stockcheck/projection.py` grades every demand order on the board curves (board draws first, orders netted in week order, receipts lift the cap and pin an earliest start), `helpers.agent_policy.apply_stock_policy` is the one patch every solver caller runs, `earliest_start_hour` is a hard floor in CP-SAT / the greedy seed / the validator (`STOCK_FLOOR`). Implementation contracts: `2026-09-01-po-stock-contracts.md`. Verified headless in the browser; suite 830 green. Known follow-ups: rounding of sub-day leads now reads in hours; the 8/24 sample feed is content-stale until IT widens the window; blanket POs and partial receipts still need IT columns (plan §3).
...
| **3 — Policy — BUILT 2026-09-15 (signed off "build slice 3")** | `demand_view[i].projected` (stockcheck/projection.py) drives the per-order trim; `earliest_start_hour` floor per order for the greedy seed / CP-SAT / validator / overnight; `[stock] solver_policy` + `solver_earliest_start` knobs | Solver stops planning inside the window and stops over-trimming later weeks | done |
