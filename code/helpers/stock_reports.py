...
# Time-phased supply verdicts (stockcheck.timeline, contract §3.5) ride on
# each schedule_view row as "supply". A report saved before the PO feed has
# none: every helper returns its quiet default and the tables look as they
# did without the feed.
SUPPLY_TEXT = {"SHORT": "SHORT", "DEPENDENT": "DEPENDENT",
               "NO_DATA": "NO DATA", "OK": "OK"}


def supply_of(block: dict) -> dict | None:
    s = block.get("supply")
    return s if isinstance(s, dict) and s.get("verdict") else None


def supply_flagged(block: dict) -> bool:
...
def supply_text(block: dict) -> str:
    """Plain-text verdict for a table cell ('' without a verdict)."""
    s = supply_of(block)
    if s is None:
        return ""
    v = str(s.get("verdict"))
    if v == "OK" and s.get("backed"):
        return "OK · backed by PO"
    if v == "DEPENDENT" and s.get("minor"):
        return "DEPENDENT · minor"
    return SUPPLY_TEXT.get(v, v.replace("_", " "))
...
def block_rows(report: dict, stamp: Callable[[float], str],
    ...
    """One row per production block of the board, worst first. A block is
    "at risk" when the flat on-hand check flags it OR the time-phased
    supply verdict does (SHORT / non-minor DEPENDENT); the Supply column
    appears only when the report carries verdicts at all."""
    blocks = report.get("schedule_view") or []
    has_supply = any(supply_of(b) is not None for b in blocks)
    rows: list[dict] = []
    for b in blocks:
        st_ = str(b.get("status") or "")
        flagged = supply_flagged(b)
        if only_risk and st_ not in RISK_STATUSES and not flagged:
            continue
        ...
        if has_supply:
            row["Supply"] = supply_text(b)
        row["_status"] = st_
        row["_supply_flagged"] = flagged
        row["_start_h"] = float(b.get("start_h") or 0)
        rows.append(row)
    rows.sort(key=lambda r: (STATUS_RANK.get(r["_status"], 9),
                             0 if r["_supply_flagged"] else 1, r["_start_h"]))
    return rows
