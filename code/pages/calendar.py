...
_cc1, _cc2, _cc3 = st.columns([2.2, 2.2, 1.2])
with _cc1:
...
# saves twice. Disk saves merge the hidden finished rows back (hiding the
# past never deletes it) and strip display-only overlays (cip_info CIPs,
# downtimes) — those are live-feed visualizations, not calendar data.
...
if isinstance(_req, dict) and _req.get("nonce") \
    ...
    if _req.get("kind") == "version":
        ...
        st.toast(f"Saved calendar_blocks.csv ({len(_out)} blocks) at {_stamp}.", icon=":material/save:")
        if n_holding:
            st.toast(f"Saved without {n_holding} held block(s) — holding cleared.", icon=":material/warning:")
        st.rerun()
...
with _lc4:
    note("2 weeks locked and ready, week 3 flexible: locking freezes every block that "
         ...
         "advances it). Export = the board as last checked, Calendar + Scorecard sheets; "
         "MO changes for VIF write-back live on Compare & Promote.")
